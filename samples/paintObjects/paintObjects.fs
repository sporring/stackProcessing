...
let private makeFixtureObjects width height depth : StreamedObject list =
    let isObject x y z =
        let smallCube =
            inRange 6 8 x && inRange 6 8 y && inRange 6 8 z

        let smallLine =
            x = 47 && y = 7 && inRange 4 27 z

        let largeCuboid =
            inRange 30 43 x && inRange 39 52 y && inRange 20 40 z

        let largeLine =
            x = 10 && y = 23 && inRange 4 43 z

        smallCube || smallLine || largeCuboid || largeLine

    let positions: Position3D<int> list =
        [ for z in 0 .. depth - 1 do
              for y in 0 .. height - 1 do
                  for x in 0 .. width - 1 do
                      if isObject x y z then
...
let main args =
    ...
    // make a 3d image from a stream of objects.
