...
module PipelineCodeGenerator =
    ...
    let private numberStatsFieldName portIndex =
                ...
                | "reducerOutput" ->
                    ...
                    | Some statsName ->
                        ...
                        | Some reducerNode when reducerNode.FunctionId = "ComputeStats" && edge.FromPort = 0 ->
                            Some statsName
                        | Some reducerNode when reducerNode.FunctionId = "Stats" && edge.FromPort = 0 ->
                            Some statsName
                        | Some reducerNode when reducerNode.FunctionId = "Expand" ->
                            ...
                            |> Option.bind (fun sourceNode ->
                                if sourceNode.FunctionId = "Stats" then
                                    numberStatsFieldName edge.FromPort
                                else
    ...
    let private scalarBinding (graph: SavedGraph) (nodesById: Map<string, SavedNode>) (scalarNamesByNodeId: Map<string, string>) (statsNamesByNodeId: Map<string, string>) (translationTableNamesByNodeId: Map<string, string>) (histogramNamesByNodeId: Map<string, string>) (quantileNamesByNodeId: Map<string, string>) (imageInfoNamesByNodeId: Map<string, string>) (chunkInfoNamesByNodeId: Map<string, string>) (serialGeometryNamesByNodeId: Map<string, string>) (node: SavedNode) =
        ...
        let value =
            ...
            | "MomentsThresholdFromHistogram" ->
                ...
                if node.FunctionId = "OtsuThresholdFromHistogram" then
                    $"scalarPlan [ {histogram.Value} ] >=> {functionName} () |> drain"
                else
    ...
    let private savedElementLine (graph: SavedGraph) (nodesById: Map<string, SavedNode>) (scalarNamesByNodeId: Map<string, string>) (statsNamesByNodeId: Map<string, string>) (translationTableNamesByNodeId: Map<string, string>) (histogramNamesByNodeId: Map<string, string>) (quantileNamesByNodeId: Map<string, string>) (imageInfoNamesByNodeId: Map<string, string>) (chunkInfoNamesByNodeId: Map<string, string>) (serialGeometryNamesByNodeId: Map<string, string>) (node: SavedNode) =
                        ...
                        |> Option.bind (fun sourceNode ->
                            if sourceNode.FunctionId = "Expand" then
                                ...
                                |> Option.map (fun recordSource ->
                                    ...
                                    elif recordSource.FunctionId = "Stats" then
                                        PortType.Custom "NumberStats"
        ...
        | "NormalNoise" ->
            ...
            $"|> zero<{pixelType}> {width} {height} {depth}{Environment.NewLine}>=> addNormalNoise {mean} {std}" |> sourcePrefix availableMemory
        | "SaltAndPepperNoise" ->
            let availableMemory = parameterValue "availableMemory"
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let width = parameterValue "width"
            ...
            $"|> zero<{pixelType}> {width} {height} {depth}{Environment.NewLine}>=> addSaltAndPepperNoise {probability} {pepper} {salt}" |> sourcePrefix availableMemory
        | "PoissonNoise" ->
            let availableMemory = parameterValue "availableMemory"
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let width = parameterValue "width"
            ...
            $"|> zero<{pixelType}> {width} {height} {depth}{Environment.NewLine}>=> addPoissonNoise {lambda}" |> sourcePrefix availableMemory
        | "SpeckleNoise" ->
            let availableMemory = parameterValue "availableMemory"
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let width = parameterValue "width"
        ...
        | "EstimateHistogram" ->
            ...
            $"|> readRandom<{pixelType}> {slices} {input} {suffix}{Environment.NewLine}>=> imageHistogram ()" |> sourcePrefix availableMemory
        | "ReadRange" ->
            let availableMemory = parameterValue "availableMemory"
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let format = savedParamValue "format" node
        ...
        | "Resize" ->
            ...
            $"|> resize {width} {height} {depth} {interpolation}"
        | "Resample" ->
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            ...
            let interpolation = quotedParameter "interpolation"
            $"|> resample {factorX} {factorY} {factorZ} {interpolation}"
        | "Repeat" ->
            let depth = parameterValue "depth"
            $">=> repeatStage {depth}"
        | "WriteChunks" ->
        ...
        | "ShowImage" ->
            ...
            let xAxis = quotedParameter "xAxis"
            let yAxis = quotedParameter "yAxis"
            $">=> show (showChunkWithLabels {colorMap} {title} {xAxis} {yAxis})"
        | "SumProjection" ->
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let functionName = savedParamValue "function" node
            $">=> sumProjection {quote functionName}"
        | "UnaryImageFunction" ->
        ...
        | "SmoothWGauss" ->
            ...
            $">=> gaussianFilter {sigma} {windowSize}"
        | "Convolve" ->
        ...
        | "FiniteDiff" ->
            ...
            | "0" | "x" -> $">=> finiteDiffX {order}"
            | "1" | "y" -> $">=> finiteDiffY {order}"
            | "2" | "z" -> $">=> finiteDiffZ {order}"
        ...
        | "SerialPolynomialBiasCorrect" ->
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let order = parameterValue "order"
            $">=> serialPolynomialBiasCorrect {order}"
        | "SerialEstTrans" ->
            let pixelType = pipelinePixelType "Float32"
            ...
            $">=> serialEstTrans {searchRadius} {method} {scale} {pixelFraction}"
        | "SerialApplyTrans" ->
            let pixelType = pipelinePixelType "Float32"
            ...
            $">=> serialApplyTrans {background} {geometry}"
        ...
        | "AddSaltAndPepperNoise" ->
            ...
            let pepper = parameterValueOrDefault "pepper" "None"
            let salt = parameterValueOrDefault "salt" "None"
            $">=> addSaltAndPepperNoise {probability} {pepper} {salt}"
        | "AddPoissonNoise" ->
            let pixelType = pipelinePixelType "Float32"
            let lambda = parameterValue "lambda"
        ...
        | "SiftKeypoints" ->
            ...
            let contrastThreshold = parameterValue "contrastThreshold"
            let stride = parameterValue "stride"
            $">=> siftKeypoints {sigma0} {scaleFactor} {scaleLevels} {contrastThreshold} {stride}"
        | "LogBlobKeypoints" ->
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let sigma = parameterValue "sigma"
            let threshold = parameterValue "threshold"
        ...
        | "HessianKeypoints" ->
            ...
            let threshold = parameterValue "threshold"
            let stride = parameterValue "stride"
            $">=> hessianKeypoints {sigma} {responseKind} {threshold} {stride}"
        | "Harris3DKeypoints" ->
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let sigma = parameterValue "sigma"
            ...
            let threshold = parameterValue "threshold"
            let stride = parameterValue "stride"
            $">=> harris3DKeypoints {sigma} {rho} {k} {threshold} {stride}"
        | "Forstner3DKeypoints" ->
            let pixelType = pixelTypeNameFromParameter "type" "Float32" node
            let sigma = parameterValue "sigma"
            let rho = parameterValue "rho"
            let threshold = parameterValue "threshold"
            let stride = parameterValue "stride"
            $">=> forstner3DKeypoints {sigma} {rho} {threshold} {stride}"
        ...
        | "StreamConnectedObjects" ->
            let connectivity = parameterValue "connectivity"
            $">=> streamConnectedObjects ObjectConnectivity.{connectivity}"
        | "Stats" ->
            ">=> stats ()"
        | "ObjectSizes" ->
            ">=> objectSizes"
    ...
    let generateSavedGraph (graph: SavedGraph) =
        ...
        let statsExpandNodesWithLinkedOutputs =
            expandNodesFor (fun node -> node.FunctionId = "ComputeStats" || node.FunctionId = "Stats")
        ...
        let statsProducerNodesWithDirectLinkedOutputs =
            ...
            |> Array.choose (fun edge ->
                if edge.FromKind = "reducerOutput" && edge.ToKind = "parameterInput" then
                    ...
                    None)
            |> Array.filter (fun node -> node.FunctionId = "ComputeStats" || node.FunctionId = "Stats")
            |> Array.distinctBy _.Id
        ...
        let statsNamesByNodeId =
            let producerNames =
                ...
                |> Array.mapi (fun index node ->
                    let prefix = if node.FunctionId = "Stats" then "NumberStats" else "ImageStats"
